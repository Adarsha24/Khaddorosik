    ...
    <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e2e6ec]">
          <span className="text-[16px] font-bold text-[#1e2433] flex items-center gap-2">
            💳 Settle Payment
          </span>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-full flex items-center justify-center text-[#8a95a8] hover:bg-[#f0f2f5] cursor-pointer text-[18px]"
          >
            ×
          </button>
        </div>
        ...
        {/* Summary */}
        <div className="bg-[#f7f8fb] px-5 py-5 text-center border-b border-[#e2e6ec]">
          <div className="text-[32px] font-bold text-[#1e2433]">₹{display.toLocaleString('en-IN')}</div>
          <div className="text-[12px] text-[#8a95a8] mt-1">Table 7 • Rahul Khanna • 3 guests</div>
          <div className="flex justify-center gap-3 mt-2 text-[11px] text-[#8a95a8]">
            <span>Subtotal: ₹1,380</span>
            <span>GST: ₹138</span>
            <span className="text-[#27ae60]">Discount: −₹69</span>
          </div>
        </div>

        {/* Payment methods */}
        <div className="px-5 pt-4">
          <div className="text-[11px] font-semibold text-[#8a95a8] uppercase tracking-[0.5px] mb-2.5">
            Payment Method
          </div>
            ...
            {PAY_METHODS.map(({ id, icon, label }) => (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1.5 py-3 rounded-xl border-2 text-[12px] font-semibold cursor-pointer transition-all
                  ${payMethod === id
                    ? 'border-[#e85c26] bg-[#fff3ee] text-[#e85c26]'
          ...
          {/* Split section */}
          {payMethod === 'split' && (
            <div className="bg-[#f7f8fb] rounded-xl p-3.5 mb-4">
              <div className="text-[11px] font-semibold text-[#8a95a8] uppercase tracking-[0.5px] mb-2">Split Payment</div>
              <div className="flex items-center justify-between mb-2 gap-3">
                <span className="text-[12px] font-medium flex items-center gap-1.5">💵 Cash</span>
                <input type="number" defaultValue="800" className="flex-1 border border-[#e2e6ec] rounded-[6px] px-2 py-1.5 text-[12px] text-right outline-none focus:border-[#e85c26]" />
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-[12px] font-medium flex items-center gap-1.5">📱 UPI</span>
                <input type="number" defaultValue="656" className="flex-1 border border-[#e2e6ec] rounded-[6px] px-2 py-1.5 text-[12px] text-right outline-none focus:border-[#e85c26]" />
              </div>
            </div>
          )}
          ...
          {/* Tip */}
          <div className="mb-4">
            <div className="text-[11px] font-semibold text-[#8a95a8] uppercase tracking-[0.5px] mb-2">Add Tip</div>
          ...
          <button
            onClick={onConfirm}
            className="w-full py-3.5 rounded-xl bg-[#e85c26] text-white text-[15px] font-bold flex items-center justify-center gap-2 hover:bg-[#c94d1d] transition-colors cursor-pointer mb-5"
          >
